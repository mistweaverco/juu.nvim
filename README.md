...
- **Bottom line / cmdline row:** `messages.set_cmdheight_zero` (default **true**) does two things: it passes `set_cmdheight` into [`vim.ui_attach()`](https://neovim.io/doc/user/lua.html#vim.ui_attach()), and after each redirected `msg_show` it **schedules** [`vim.o.cmdheight = 0`](https://neovim.io/doc/user/options.html#'cmdheight'). The schedule matters if you use [`vim._core.ui2`](https://neovim.io/doc/user/lua.html#vim._core.ui2): ui2’s own `vim.ui_attach` uses `set_cmdheight = false`, so Neovim often **keeps** a non-zero `cmdheight` unless something sets it back to 0 (which also lets ui2’s `OptionSet` handler hide the cmd float). Set `messages = { set_cmdheight_zero = false }` if you want a persistent bottom row.
