...
local M = {}
local spinner = require("juu.spinner")
...
M.options = {
  ...
  ---
  --- See also: |juu.spinner.Manga| and |juu.spinner.Anime|.
  ---
  ...
  --->lua
  ---     function juu.display.default_format_message(msg)
  ---       local message = msg.message
  ---       if not message then
  ---         message = msg.done and "Completed" or "In progress..."
  ---       end
  ...
  ---         priority = 60,
  ---         icon = juu.progress.display.for_icon(juu.spinner.animate("triangle", 3), "💯"),
  ---         skip_history = false,
  ---       },
  ---       rust_analyzer = {
  ---         name = "Rust Analyzer",
  ---         icon = juu.progress.display.for_icon(juu.spinner.animate("arrow", 2.5), "🦀"),
  ---         update_hook = function(item)
  ---           require("juu.notify").set_content_key(item)
...
---@options ]]
require("juu.options").declare(M, "progress.display", M.options)
