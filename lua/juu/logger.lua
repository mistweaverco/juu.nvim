...
local PLUGIN_PATH_PATTERN = "(/lua/juu.+)"
...
M.options = {
  ...
  --- footprint of Juu logs is approximately twice `logger.max_size`.
  ...
  --- Juu, run only one instance of Neovim, or set this option to `false`.
  ---
  ---@type number | false
  max_size = 10000,
  ...
  --- Where Juu writes its logs to
  ---
  --- Using `vim.fn.stdpath("cache")`, the default path usually ends up at
  --- `~/.cache/nvim/juu.nvim.log`.
  ---
  ---@type string
  path = string.format("%s/juu.nvim.log", vim.fn.stdpath("cache")),
}
---@options ]]

require("juu.options").declare(M, "logger", M.options, function()
  ...
  if M.options.max_size then
    ...
    if log then
      ...
      vim.notify("Could not open Juu log: " .. M.options.path, vim.log.levels.WARN)
    end
  end
end)
...
--- Open the Juu log; prune the existing log if it has grown too large.
---
