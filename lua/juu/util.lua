...
M.add_title_to_win = function(winid, title, opts)
  ...
  if title_winid and vim.api.nvim_win_is_valid(title_winid) then
    ...
    elseif opts.align ~= "left" then
      vim.notify(string.format("Unknown juu window title alignment: '%s'", opts.align), vim.log.levels.ERROR)
    end
    ...
    vim.cmd(string.format(
      [[
      autocmd WinClosed %d ++once lua require('juu.util')._on_win_closed(%d)
    ]],
      winid,
      winid
    ))
  ...
  local ns = vim.api.nvim_create_namespace("JuuWindow")
