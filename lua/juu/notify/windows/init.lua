local api = vim.api
local animate = require("juu.notify.animate")
...
---@field notif_bufs table<number, JuuNotificationBuf>
---@field timers table
---@field stages table
  ...
  for field, goal in pairs(goals) do
    if field ~= "time" then
      ...
      elseif goal_type ~= "table" then
        ...
        error("juu.notify: Invalid stage goal: " .. vim.inspect(goal))
      end
    end
  end
