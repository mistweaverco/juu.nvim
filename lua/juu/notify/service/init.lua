local JuuNotificationBuf = require("juu.notify.service.buffer")
local util = require("juu.notify.util")

---@class NotificationService
---@field private _running boolean
---@field private _pending FIFOQueue
---@field private _animator WindowAnimator
---@field private _buffers table<integer, JuuNotificationBuf>
---@field private _fps integer
local JuuNotificationService = {}

---@class juu.notify.ServiceConfig
---@field fps integer

---@param config juu.notify.ServiceConfig
function JuuNotificationService:new(config, animator)
  local service = {
    _config = config,
    _fps = config.fps(),
    _animator = animator,
...
function JuuNotificationService:_run()
  self._running = true
...
---@param notif juu.notify.Notification;q
---@return integer
function JuuNotificationService:push(notif)
  local buf = vim.api.nvim_create_buf(false, true)
  local notif_buf = JuuNotificationBuf(buf, notif, { config = self._config })
  notif_buf:render()
...
function JuuNotificationService:replace(id, notif)
...
function JuuNotificationService:dismiss(opts)
...
function JuuNotificationService:pending()
  return self._pending:length()
end

---@return NotificationService
return function(config, animator)
  return JuuNotificationService:new(config, animator)
end
