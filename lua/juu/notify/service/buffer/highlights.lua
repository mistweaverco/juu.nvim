...
---@field buffer number
---@field _config table
local JuuNotifyBufHighlights = {}

function JuuNotifyBufHighlights:new(level, buffer, config)
  local function linked_group(section)
    local orig = "JuuNotify" .. level .. section
    if vim.fn.hlID(orig) == 0 then
      orig = "JuuNotifyINFO" .. section
    end
...
function JuuNotifyBufHighlights:_redefine_treesitter()
  local buf_highlighter = require("vim.treesitter.highlighter").active[self.buffer]

  if not buf_highlighter then
    return
  end
  local render_namespace = vim.api.nvim_create_namespace("juu.notify.treesitter.override")
...
function JuuNotifyBufHighlights:set_opacity(alpha)
  if
...
function JuuNotifyBufHighlights:get_opacity()
  return self.opacity
end

---@return JuuNotifyBufHighlights
return function(level, buffer, config)
  return JuuNotifyBufHighlights:new(level, buffer, config)
end
