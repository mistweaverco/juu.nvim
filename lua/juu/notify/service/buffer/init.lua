...
local JuuNotifyBufHighlights = require("juu.notify.service.buffer.highlights")

---@class JuuNotificationBuf
---@field highlights JuuNotifyBufHighlights
---@field _config table
---@field _notif juu.notify.Notification
---@field _state "open" | "closed"
---@field _buffer number
---@field _height number
---@field _width number
---@field _max_width number | nil
local JuuNotificationBuf = {}
...
function JuuNotificationBuf:new(kwargs)
  local notif_buf = {
...
function JuuNotificationBuf:set_notification(notif)
  self._notif = notif
  self:_create_highlights()
end

function JuuNotificationBuf:_create_highlights()
  local existing_opacity = self.highlights and self.highlights.opacity or 100
  self.highlights = JuuNotifyBufHighlights(self._notif.level, self._buffer, self._config)
  if existing_opacity < 100 then
    self.highlights:set_opacity(existing_opacity)
  end
end

function JuuNotificationBuf:open(win)
...
function JuuNotificationBuf:should_animate()
  return self._notif.animate
end

function JuuNotificationBuf:close(win)
...
function JuuNotificationBuf:height()
  return self._height
end

function JuuNotificationBuf:width()
  return self._width
end

function JuuNotificationBuf:should_stay()
...
function JuuNotificationBuf:render()
  local notif = self._notif
  local buf = self._buffer

  local render_namespace = require("juu.notify.render.base").namespace()
...
function JuuNotificationBuf:timeout()
  return self._notif.timeout
end

function JuuNotificationBuf:buffer()
  return self._buffer
end

function JuuNotificationBuf:is_valid()
  return self._buffer and vim.api.nvim_buf_is_valid(self._buffer)
end

function JuuNotificationBuf:level()
  return self._notif.level
end

---@param buf number
---@param notification juu.notify.Notification;q
---@return JuuNotificationBuf
return function(buf, notification, opts)
  return JuuNotificationBuf:new(
