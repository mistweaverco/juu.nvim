--- Declarative options for this plugin (not specific to Juu).
local M = {}
local health = require("juu.health")

---@class juu.DeprecatedOption<T> : { default_value: T, deprecated_option: string|true }

---@generic T
---@param default_value T
---@param advice string|nil
---@return juu.DeprecatedOption<T>
...
--- Declare a table as a juu module, so we can tell using is_juu_module().
local function set_juu_module(m)
...
--- Whether this is juu module, i.e., set_juu_module() was called on it.
local function is_juu_module(m)
  if type(m) ~= "table" then
    return false
  end
  local mt = getmetatable(m)
  return mt and mt.juu_module == true
end
