local global_config = require("juu.config")
local map_util = require("juu.map_util")
local patch = require("juu.patch")
local util = require("juu.util")
local M = {}

---@alias juu.Mode "insert" | "visual" | "normal" | "select"

---@class (exact) juu.InputContext
---@field opts? juu.InputOptions
---@field on_confirm? fun(text?: string)
---@field winid? integer
---@field history_idx? integer
---@field history_tip? string
---@field mode_to_restore? juu.Mode The mode to restore when the input window closes.

---@class (exact) juu.InputConfig
---@field start_in_insert? boolean
---@field start_mode? juu.Mode
...
local keymaps = {
  ...
  },
  {
    ...
    plug = "<Plug>JuuInput:HistoryPrev",
    rhs = function()
      M.history_prev()
    end,
  },
  {
    ...
    plug = "<Plug>JuuInput:HistoryNext",
    rhs = function()
      M.history_next()
    end,
  },
...
local function apply_highlight()
  ...
  local ns = vim.api.nvim_create_namespace("JuuHighlight")
  local highlights = {}
  ...
  if findstart == 1 then
    ...
    if pieces[1] == "custom" or pieces[1] == "customlist" then
      ...
      if ok then
        ...
        vim.api.nvim_err_writeln(string.format("juu.nvim: unsupported completion method '%s'", completion))
        return {}
      end
    end
  end
...
---@param opts string|juu.InputOptions
...
local show_input = util.make_queued_async_fn(2, function(opts, on_confirm)
  ...
  local config = global_config.get_mod_config("input", opts) --[[@as juu.InputConfig]]
  ...
  for mode, user_maps in pairs(config.mappings) do
    map_util.create_maps_to_plug(bufnr, mode, user_maps, "JuuInput:")
  end
  ...
  if opts.completion then
    vim.bo[bufnr].completefunc = "v:lua.require'juu.input'.completefunc"
    vim.bo[bufnr].omnifunc = "v:lua.require'juu.input'.completefunc"
