...
local function clear_callback()
  _G.juu_fzf_choice = function() end
  _G.juu_fzf_cancel = function() end
end
...
M.select = function(config, items, opts, on_choice)
  ...
  _G.juu_fzf_cancel = function()
    clear_callback()
    on_choice(nil, nil)
  end
  _G.juu_fzf_choice = function(label)
    clear_callback()
  ...
  vim.fn["juu#fzf_run"](labels, string.format('--prompt="%s "', opts.prompt), config.window)
  -- fzf doesn't have a cancel callback, so we have to make one.
  vim.cmd([[autocmd TermClose <buffer> ++once lua require('juu.select.fzf')._on_term_close()]])
end
