...
M.select = function(config, items, opts, on_choice)
  ...
  local ns = vim.api.nvim_create_namespace("juuSelect")
  for _, hl in ipairs(highlights) do
    local lnum, end_col = unpack(hl)
    vim.api.nvim_buf_add_highlight(bufnr, ns, "JuuSelectIdx", lnum, 0, end_col)
  end
  ...
  vim.bo[bufnr].filetype = "JuuSelect"
  if vim.fn.has("nvim-0.9") == 0 then
    util.add_title_to_win(winid, opts.prompt)
  end

  map_util.create_plug_maps(bufnr, keymaps)
  map_util.create_maps_to_plug(bufnr, "n", config.mappings, "JuuSelect:")
